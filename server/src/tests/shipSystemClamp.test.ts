...
describe('ShipSystem cave clamp', () => {
  it('clamps ship inside cave and applies wall damage', () => {
    ...
    state.ship.position.z = 200;
    const room = {
      state,
      inputs: new Map([['pilot', { seat: 'pilot', move: { x: 1, y: 0 }, lift: 0.5 }]]),
      rng: () => 0.5,
      simulationTime: 1000,
      stabilizerUntil: 0,
    ...
    system.update(0.1);
    expect(Math.hypot(state.ship.position.x, state.ship.position.y, state.ship.position.z)).toBeLessThan(600);
